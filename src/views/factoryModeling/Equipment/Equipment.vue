	...
	<div class="app-container">
		<el-container>
			<el-aside width="300px" style="padding-top: 20px;">
					...
					<el-form :model="searchFormData" size="small">
						<el-row>
							<el-col :span="24">
								<el-form-item label="">
									<el-col :span="16">
										<el-input v-model="searchFormData.searchTextValue" type="text" placeholder="部門编码或名称查询" @keyup.enter.native="orgTableSearch()" />
									</el-col>
				...
				<el-row>
					<el-table ref="orgTable" :data="orgDataList" size="small" style="width: 100%" row-key="id" border lazy :load="load"
					 :tree-props="{children: 'children', hasChildren: 'leaf'}" @row-click="orgRowClick">
			...
			<el-container>
				<el-main style="padding-top: -250px;">
					<el-form :model="dataForm" @keyup.enter.native="getDataList()">
						<el-row>
							...
							<el-col :offset="8" :span="8">
								<el-form-item label="">
									<el-col :span="18">
										<el-input v-model="dataForm.key" type="text" size="small" placeholder="设备编码或名称查询" @keyup.enter.native="getDataList()" />
									</el-col>
									<el-col :offset="1" :span="3">
					...
					<el-table v-loading="dataListLoading" :data="dataList" border size="small" style="width: 100%">
						...
						<el-table-column prop="deviceIp" header-align="center" align="center" label="设备IP" />
                ...
                <el-tag v-if="scope.row.status === 3" size="small" type="warning">维修</el-tag>
              	...
              	<el-tag v-else size="small">运行</el-tag>
						...
						<el-table-column align="center" label="操作" width="100" fixed="right">
							<template slot-scope="scope">
								<el-button size="small" icon="el-icon-edit" type="primary" circle @click="addOrUpdateHandle('2', scope.row.id)" />
								<el-button size="small" icon="el-icon-delete" type="danger" circle @click="deleteHandle(scope.row)" />
							</template>
						</el-table-column>
					</el-table>
		...
		<el-dialog title="设备导入" size="small" :close-on-click-modal="false" :visible.sync="visible" width="75%">
	...
	export default {
		...
		methods: {
			...
			beforeUpload(file) {
			  ...
			  fd.append('file', file);
			  importEquipments(fd).then(response => {
