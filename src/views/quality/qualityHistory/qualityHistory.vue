  ...
  <div class="app-container">
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="">
              ...
              <el-button type="primary" icon="el-icon-receiving" round @click="visible = true">查看附件</el-button>
            </el-form-item>
          </el-col>
    ...
    <el-row>
      <el-table ref="taskTable" :data="taskDataList" size="small" row-key="taskId" border lazy :load="load" :tree-props="{children: 'children', hasChildren: 'leaf'}">
        ...
        <el-table-column prop="taskStatusName" align="center" label="二维码" />
        <el-table-column prop="taskPlannedNum" align="center" label="检验类型" />
        <el-table-column prop="taskProductionNum" align="center" label="检验数量" />
        <el-table-column prop="taskWorkshopName" align="center" label="检验结果" />
        <el-table-column prop="taskScheduledEndTime" align="center" label="送检日期" width="150">
          <template slot-scope="scope">
            <span>{{ dateFormat(scope.row.taskScheduledEndTime) }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="taskPriorityName" align="center" label="送检人" />
    ...
    <el-dialog title="文件上传" size="small" :close-on-click-modal="false" :visible.sync="visible" width="75%">
      <el-upload ref="upload" class="upload-demo" :file-list="fileList" :on-change="handleChange" :action="uploadUrl" :show-file-list="true" :on-success="onSuccess" :on-error="onError" :auto-upload="false">
