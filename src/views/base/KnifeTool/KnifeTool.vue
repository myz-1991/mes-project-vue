<template>
	<div class="app-container">
		<el-form :model="dataForm" @keyup.enter.native="getDataList()">
            ...
            <el-button type="primary" icon="el-icon-edit-outline" size="small" round @click="visible = true">导入</el-button>
				...
				<el-col :offset="8" :span="8">
					<el-form-item label="">
						<el-col :span="18">
							<el-input type="text" size="small" v-model="dataForm.key" placeholder="刀具编码或名称查询" @keyup.enter.native="getDataList()"></el-input>
						</el-col>
		...
		<el-table :data="dataList" border v-loading="dataListLoading" size="small" style="width: 100%">
			...
			<el-table-column prop="code" header-align="center" align="center" label="刀具编码">
			</el-table-column>
			<el-table-column prop="name" header-align="center" align="center" label="刀具名称">
			</el-table-column>
			<!-- <el-table-column prop="mateDesignNumber" header-align="center" align="center" label="刀具类型">
      </el-table-column> -->
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				findToolPage(this.dataForm.key, this.pageSize, this.pageIndex).then(response => {
					if (response) {
			...
			addOrUpdateHandle(workType, id) {
				...
				this.$nextTick(() => {
					this.$refs.knifeToolAddOrUpdate.init(workType, id)
				})
			},
			// 删除
			deleteHandle(row) {
