					...
					<el-form :model="dataForm" size="small">
						<el-row>
							...
							<el-col :offset="8" :span="8">
								<el-form-item label="">
									<el-col :span="18">
										<el-input v-model="dataForm.searchTextValue" placeholder="坩锅号或炉批号查询" type="text" @keyup.enter.native="refreshUserTable()" />
									</el-col>
				...
				<el-row>
					<el-table ref="userList" v-loading="dataListLoading" :data="dataList" size="small" border style="width: 100%;">
						...
						<el-table-column align="center" label="操作" width="200">
							<template slot-scope="scope">
								<el-button size="mini" icon="el-icon-s-tools" type="success" round @click="updateCrucible(scope.row)"></el-button>
	...
	export default {
		...
		methods: {
			...
			updateCrucible(row) {
				...
				row.jobId = ''
				row.jobName = ''
				updateCrucible(row).then(response => {
					this.$message({
						message: '操作成功',
						type: 'success',
						onClose: () => {
