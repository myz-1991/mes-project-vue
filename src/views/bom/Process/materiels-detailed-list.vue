<template>
  <el-dialog :title="titleText" :close-on-click-modal="false" :visible.sync="visible" width="80%">
    <el-form size="small" style="margin-bottom: 5px;" @keyup.enter.native="getDataList()">
    ...
    <el-table ref="processsTable" v-loading="dataListLoading" size="small" :data="dataList" border style="width: 100%">
      ...
      <el-table-column header-align="center" align="center" label="物料名称" width="150">
        <template slot-scope="scope">
          <el-select
            ...
            v-model="scope.row.mateId"
            size="small"
            placeholder="请选择"
            filterable
            :filter-method="mateSelectionFilter"
            style="width: 100%;"
            @change="mateSelection(scope.row.mateId, scope.$index)"
          >
      ...
      <el-table-column header-align="center" align="center" label="单套数量" width="110">
        <template slot-scope="scope">
          <el-input v-show="scope.row.show" v-model="scope.row.detaRequirementNum" size="small" placeholder="单套数量" style="width: 100%;" />
